 ...
 * @param  {string}   output   The file to store it
 * @param  {Function} callback Callback
 */
function downloadMedia (uri, output, callback) {
  if (!uri || !output) {
    callback(new Error('uri and output filename required'))
  }
  var options = {
    'url': uri,
    'headers': {
  ...
  request(options).pipe(fs.createWriteStream(output)).on('close', callback)
}
 ...
 * @param  {Function} callback Callback
 */
function downloadDocument (uri, callback) {
  if (!uri) {
    callback(new Error('uri required'))
  }
  var options = {
    'url': uri,
    'headers': {
